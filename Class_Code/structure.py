...
def plot_epipolar_lines(p1, p2, F, fig, ax1, ax2, show_epipole=False):
    ...
    # L1 = F * p2
    fig, ax1 = plot_epipolar_line(p1, p2, F, fig, ax1, show_epipole)
    # Plot the epipolar lines on img2 with points p1 from the left side
    # L2 = F' * p1
    fig, ax2 = plot_epipolar_line(p2, p1, F.T, fig, ax2, show_epipole)
    return fig,ax1, ax2


def plot_epipolar_line(p1, p2, F, fig, ax, show_epipole=False):
    ...
    for line in lines.T:
        ...
        ax.plot(xpts[valid_idx], ypts[valid_idx], linewidth=1, alpha=0.4)
        ax.plot(p1[0], p1[1], 'ro', markersize=3, alpha=0.1)

    if show_epipole:
        epipole = compute_epipole(F)
        ax.plot(epipole[0] / epipole[2], epipole[1] / epipole[2], 'r*')
    return fig, ax
